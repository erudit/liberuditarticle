...
class TestGetKeywords:
    ...
    def test_can_return_equivalent_keywords_in_the_lang_order_defined_in_the_article(
        self, value
    ):  # noqa
        assert tuple(value.keys()) == (
            "fr",
...
class TestSectionTitle(object):
    def test_can_return_section_titles(self):
        value = self.test_objects["1027883ar.xml"].get_section_titles(level=1)
        assert value == {
            "main": "Paroles et points de vue atikamekw nehirowisiwok",
            "paral": collections.OrderedDict(),
        }

        value = self.test_objects["1027883ar.xml"].get_section_titles(level=2)
    ...
    def test_can_return_multilingual_section_titles(self):
        value = self.test_objects["1030384ar.xml"].get_section_titles(level=1)

        paral = collections.OrderedDict()
        paral[
            "en"
        ] = "Special Edition: Undertaking and innovating in a globalised economy..."
        paral[
            "es"
        ] = "Dossier especial: Emprender e innovar en una economía globalizada…"  # noqa
        ...
        assert value == [
            {
                ...
                "scope": "numero",
                "content": [
                    "Une version antérieure de ce texte à été publiée dans <em>Le Devoir </em>du 7 mars 2011."
                ],  # noqa
            }
        ]
        ...
        assert value == [
            {
                ...
                "scope": "numero",
                "content": [
                    'Pour obtenir la liste des sigles utilisés dans cet article et les références complètes aux oeuvres de Marie-Claire Blais, <a href="http://www.erudit.org/revue/vi/2011/v37/n1/1006456ar.html">voir p. 7</a>.'
                ],  # noqa
            }
        ]
        ...
        assert value == [
            {
                ...
                "scope": "numero",
                "content": [
                    "Le présent texte suscite un questionnement approfondi sur l’économie du contrat. Le lecteur trouvera des éléments de réponse dans le texte suivant : « L’apport épistémologique de la notion d’économie du contrat en matière d’interprétation », (2016) 57:4 C de D à paraître en décembre."
                ],  # noqa
            },
            {
                "type": "auteur",
                "scope": None,
                "content": [
                    "Je tiens à remercier les professeurs André Bélanger (Université Laval) et Martin Ndende (Université de Nantes) sous la direction desquels s’effectue ma thèse intitulée « L’économie du contrat dans l’effet obligatoire des clauses du contrat : l’exemple du contrat de transport »."
                ],  # noqa
            },
        ]
...
class TestArticleSavantComplet(object):
    ...
    def test_html_title(self):
        assert (
            self.test_objects["1001948ar.xml"].get_html_title()
            == "La précision des analystes financiers en Europe : l’effet pays et l’effet secteur revisités"
        )  # noqa
        assert (
            self.test_objects["1001948ar_alt.xml"].get_html_title()
            == "La précision des analystes financiers en Europe : l’effet pays et l’effet secteur <strong>test</strong> test 2 revisités"
        )  # noqa
        # Check that <liensimple> tags are not stripped or displayed as links. They should be
        # displayed as text.
        assert (
            self.test_objects["007801ar.xml"].get_html_title()
            == "Sites Internet des Archives nationales du Canada et des Archives nationales du Québec &lt;http://www.archives.ca&gt; et &lt;http://www.anq.gouv.qc.ca&gt;. Sites évalués à la fin mars 2003."
        )  # noqa

    def test_publication_period(self):
        assert (
            self.test_objects["1001948ar.xml"].get_publication_period() == "Juin 2010"
        )
        assert (
            self.test_objects["009255ar.xml"].get_publication_period()
            == "November 2003, February 2004"
        )
        assert (
            self.test_objects["1005860ar.xml"].get_publication_period() == "2008–2009"
        )

    def test_can_return_titles_subtitles(self):
        assert self.test_objects["1005860ar.xml"].get_titles() == {
            "main": Title(
                title="Esthétique et sémiotique", subtitle="Présentation", lang="fr"
            ),  # noqa
            "paral": [
                Title(
                    title="Aesthetics and Semiotics", lang="en", subtitle="Presentation"
                )  # noqa
            ],
            "equivalent": [],
            "reviewed_works": [],
        ...
        assert self.test_objects["1004725ar.xml"].get_titles() == {
            "main": Title(
                title="Introduction: Food, Language, and Identity",
                subtitle=None,
                lang="en",
            ),  # noqa
            "paral": [],
            "equivalent": [
                Title(title="Cuisine, langue et identité", subtitle=None, lang="fr")
            ],  # noqa
            "reviewed_works": [],
        }
    ...
    def test_can_return_formatted_titles(self):
        ...
        assert (
            self.test_objects["1005860ar.xml"].get_title(formatted=True, html=True)
            == EXPECTED
        )
        ...
        assert (
            self.test_objects["044308ar.xml"].get_title(formatted=True, html=True)
            == EXPECTED
        )
        ...
        assert (
            self.test_objects["1056263ar.xml"].get_title(formatted=True, html=True)
            == EXPECTED
        )
        ...
        assert (
            self.test_objects["1067047ar.xml"].get_title(formatted=True, html=True)
            == EXPECTED
        )
    ...
    def test_can_return_journal_titles(self):
        ...
        assert self.test_objects["044308ar.xml"].get_journal_titles() == {
            ...
            "paral": [],
            "equivalent": [
                Title(title="Industrial Relations", subtitle=None, lang="en")
            ],
        }

    def test_can_return_its_formatted_journal_title(self):
        assert (
            self.test_objects["1005860ar.xml"].get_formatted_journal_title()
            == "Recherches sémiotiques / Semiotic Inquiry"
        )  # noqa
        assert (
            self.test_objects["044308ar.xml"].get_formatted_journal_title()
    ...
    def test_can_return_references(self):
        ...
        assert references[0] == {
            "title": "Akenside, Mark. Poems. London: J. Dodsley, 1772.",
            "doi": None,
        }  # noqa
        ...
        assert references[3] == {
            "title": "Cheung, Martha P. Y. (2005): ‘To translate’ means ‘to exchange’? A new interpretation of the earliest Chinese attempts to define translation (‘fanyi’). Target. 17(1):27-48.",
            "doi": "10.1075/target.17.1.03che",
        }  # noqa

    def test_can_return_references_with_markup(self):
        references = self.test_objects["009255ar.xml"].get_references()
        assert references[0] == {
            "title": "Akenside, Mark. <em>Poems</em>. London: J. Dodsley, 1772.",
            "doi": None,
        }  # noqa
        assert len(references) == 53
    ...
    def test_html_body_with_empty_alinea(self):
        ...
        assert (
            "L’historien Pierre Gaxotte pensa proposer la candidature d’Anouilh" in html
        )

    def test_breaking_spaces_are_not_stripped_from_html_body(self):
        html = self.test_objects["1065852ar.xml"].get_html_body()
        assert (
            "Erratum\xa0: Une erreur s’est glissée dans le volume 52.1 de "
    ...
    def test_get_formatted_title(self):
        ...
        assert (
            self.test_objects["1056361ar.xml"].get_formatted_title()
        ...
        assert (
            self.test_objects["1056263ar.xml"].get_formatted_title()
            == "Colloque d’histoire antillaise : CENTRE D’ENSEIGNEMENT SUPÉRIEUR LITTÉRAIRE DE POINTE-À-PITRE (25-28 avril 1969)"
        )  # noqa
...
class TestArticleSavantMinimal(object):
    def test_can_return_titles_and_subtitles(self):
        assert self.test_objects["602354ar.xml"].get_titles(html=False) == {
            "main": Title(
                title="Immigration, langues et performance économique : le Québec et l’Ontario entre 1970 et 1995",
                lang="fr",
                subtitle=None,
            ),  # noqa
            "equivalent": [
                Title(
                    title="Immigration, Languages and Economic Performance: Quebec and Ontario between 1970 and 1995",
                    lang="en",
                    subtitle=None,
                )  # noqa
            ],
    ...
    def test_get_formatted_title(self):
        ...
        assert (
            self.test_objects["1054095ar.xml"].get_formatted_title()
        ...
        assert (
            self.test_objects["004812ar.xml"].get_formatted_title()
            == "The Tardy, Tasty and Chilly Thermophiles of the Champlain Sea"
        )  # noqa
    ...
    def test_get_formatted_journal_title(self):
        ...
        assert (
            self.test_objects["301900ar.xml"].get_formatted_journal_title()
            == "Revue d'histoire de l'Amérique française"
        )  # noqa

    def test_get_title(self):
        assert (
            self.test_objects["1055849ar.xml"].get_title(formatted=True, html=True)
...
@with_fixtures(
    "./eruditarticle/tests/fixtures/article/format_person_name/", EruditArticle
)
...
class TestArticleCulturelMinimal(object):
    ...
    def test_can_return_its_title(self):
        assert self.test_objects["49222ac.xml"].get_titles(html=False) == {
            ...
            "equivalent": [],
            "reviewed_works": [
                "Love and Death on Long Island (Rendez-vous à Long Island), Grande-Bretagne / Canada, 1997, 93 minutes"
            ],  # noqa
        }
    ...
    def test_can_return_its_formatted_title(self):
        assert (
            self.test_objects["90859ac.xml"].get_title(formatted=True, html=True)
            == "Doubles vies : écris-moi des mots qui sonnent, sonnent, sonnent"
        )  # noqa
        assert (
            self.test_objects["90873ac.xml"].get_title(formatted=True, html=True)
            == "God exists, Her Name Is Petrunya : à la fin, c’est l’eau qui gagne"
        )  # noqa
    ...
    def test_publication_period(self):
        assert (
            self.test_objects["34598ac.xml"].get_publication_period()
            == "Juin–Juillet–Août 2008"
        )
        assert (
            self.test_objects["49222ac.xml"].get_publication_period() == "Mai–Juin 1998"
        )
        assert (
            self.test_objects["65943ac.xml"].get_publication_period()
            == "Février–Mars–Avril–Mai 2012"
        )

    def test_can_extract_reviewed_works(self):
        assert self.test_objects["49222ac.xml"].get_reviewed_works(html=False) == [
            "Love and Death on Long Island (Rendez-vous à Long Island), Grande-Bretagne / Canada, 1997, 93 minutes"
        ]  # noqa
