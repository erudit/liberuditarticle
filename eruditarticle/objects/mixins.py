...
class CopyrightMixin(object):
    ...
    def _get_copyrights_label(self, root, language, html=False):
        ...
        if len(declaration):
            labels = declaration[0].xpath(
                'label[@lang="{}"]'.format(language)
            ) or declaration[0].findall("label")
        if len(labels):
            return (
                self.convert_marquage_content_to_html(labels[0])
                if html
                else labels[0].text
            )
        else:
    ...
    def _get_copyrights_names(self, root, html=False):
        ...
        for name in names:
            formatted_names.append(
                " ".join(
                    [
                        self.convert_marquage_content_to_html(part)
                        if html
                        else part.text
                        for part in name
                    ]
